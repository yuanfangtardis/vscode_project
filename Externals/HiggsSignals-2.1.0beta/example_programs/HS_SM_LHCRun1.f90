!--------------------------------------------------------------------------------------
! This example program is part of HiggsSignals (TS 04/06/2017).
!--------------------------------------------------------------------------------------
program HS_SM_LHCRun1
! In this example we scan the mass of the SM Higgs boson and evaluate the chi^2
! from the ATLAS+CMS Run1 combination. We run over the three available mass pdfs.
! Results can be plotted in the /results/ folder with the python script plot_LS_SM_mass.py.
!--------------------------------------------------------------------------------------
...
&                    get_peakchi2, print_corr_mu_to_file 
 ...
 integer :: nHzero, nHplus, ndf, i, j, k, ii, jj, CPvalue
...
! call setup_assignmentrange(Lambda)
 call setup_assignmentrange_LHCrun1(Lambda)
...
! with Mh out of range [0.8 GeV, 500 GeV]. The calculation is then bypassed.
   if(.not. (SMGammaTotal .lt. 0)) then
    ...
    call HiggsBounds_neutral_input_properties(Mh,SMGammaTotal,CPvalue)
