...
!------------------------------------------------------------
 ...
 call read_matrix_from_file(5,pathname_HS//"XScov_13TeV.in",delta_rate%CS13cov, XSmodel)
 call read_matrix_from_file(5,pathname_HS//"XScovSM_13TeV.in",delta_rate%CS13covSM, XSSM)
...
!------------------------------------------------------------
 ...
 call read_matrix_from_file(5,filename_XS13,delta_rate%CS13cov, XSmodel)
...
subroutine run_HiggsSignals_LHC_Run1_combination(Chisq_mu, Chisq_mh, Chisq, nobs, Pvalue)
 ...
 do n=1,ndat
  ...
  call evaluate_LHC_Run1_combination(theo(n),HSres(n))       
 ...
 if(output_level.ne.0) then    
  ...
  write(*,*) '#*************************************************************************#' 
  write(*,'(A55,F21.8)') 'chi^2 from signal rate observables = ',HSres(n)%Chisq_mu
  write(*,'(A55,F21.8)') 'chi^2 from Higgs mass observables = ',HSres(n)%Chisq_mh
  write(*,'(A55,F21.8)') 'chi^2 (total) = ',HSres(n)%Chisq
  write(*,'(A55,I21)') 'Number of signal rate observables = ',&
...
!------------------------------------------------------------
subroutine evaluate_LHC_Run1_combination( t , r )
!------------------------------------------------------------
!
!------------------------------------------------------------
 use usefulbits, only : np,Hneut,Hplus,dataset,results, vsmall
 use usefulbits_hs, only : HSresults, output_level, Nparam, &
& LHCrun1_rates, LHCrun1_correlationmatrix, useaveragemass, &
& assignmentrange_LHCrun1
 ...
 double precision :: dmobs = 0.237D0
 ...
 do i=lbound(LHCrun1_rates,dim=1),ubound(LHCrun1_rates,dim=1)
  ...
  ncomb = 0
  do k=1,np(Hneut)
   if(abs(t%particle(Hneut)%M(k)-mobs).le.&
&     abs(assignmentrange_LHCrun1*dmobs + t%particle(Hneut)%dM(k)) ) then
    Higgs_signal_k = signalrate(k,p,d)
...
&                 LHCrun1_rates(i)%dr0 * LHCrun1_rates(j)%dr0
 ...
 if(mav.lt.vsmall) then
  ...
  r%Chisq_mh=csq_mh(mav,mobs,dmav,dmobs)
 endif
 
 if((r%Chisq_mh+sum(csq_mu)).gt.sum(csq_mu_max)) then
  r%Chisq_mu=sum(csq_mu_max)
  r%Chisq_mh=0.0D0  
 else  
  r%Chisq_mu=sum(csq_mu)
 endif
  
 r%Chisq= r%Chisq_mu +  r%Chisq_mh
 r%nobs=20
...
!------------------------------------------------------------ 
 ...
 &  SMBR_Hgg
  ...
  double precision :: signalrate, production_rate, decay_rate, mass
  ...
  if(mutab%collider.eq.'LHC') then
   if(abs(mutab%energy-7.0D0).le.small) then
    ...
    if(p.eq.1) then 
     ...
     rate_SMref=XS_lhc13_HW_SM(refmass)     
     mutab%channel_description(i,1)='HW'     
    else if(p.eq.4) then
     rate=t%lhc13%XS_hjZ_ratio(j)  
     SMrate=t%lhc13%XS_HZ_SM(j)
     rate_SMref=XS_lhc13_HZ_SM(refmass)     
     mutab%channel_description(i,1)='HZ'       
    else if(p.eq.5) then
