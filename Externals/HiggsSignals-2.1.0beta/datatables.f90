...
!--------------------------------------------------------------------
 subroutine setup_LHC_Run1_combination
!--------------------------------------------------------------------
  use usefulbits, only : file_id_common3
  use usefulbits_HS, only : LHCrun1_rates, LHCrun1_correlationmatrix
  use store_pathname_HS

  integer :: ios, n, skip, i,j, id
  double precision :: r, r_low, r_up, dble_tmp
  character(LEN=100) :: string_tmp
  
  skip =3
   open(file_id_common3, file=trim(adjustl(pathname_HS))//'Expt_tables/'// &
&	 'LHC_combination_Run1/LHC_comb_Run1.exp')

  n=0
  do
   n = n+1
   if(n.le.skip) then
    read(file_id_common3,*, iostat=ios) 
   else
    read(file_id_common3,*, iostat=ios) id, r_low, r, r_up
    if(ios.ne.0) exit
    LHCrun1_rates(n-skip)%channel_id = id
    LHCrun1_rates(n-skip)%r_low = r_low
    LHCrun1_rates(n-skip)%r = r
    LHCrun1_rates(n-skip)%r_up = r_up
	LHCrun1_rates(n-skip)%dr_low = LHCrun1_rates(n-skip)%r - LHCrun1_rates(n-skip)%r_low
	LHCrun1_rates(n-skip)%dr_up = LHCrun1_rates(n-skip)%r_up - LHCrun1_rates(n-skip)%r       
   endif
  enddo
  ...
  n = 0
   ...
   open(file_id_common3, file=trim(adjustl(pathname_HS))//'Expt_tables/'// &
&	 'LHC_combination_Run1/LHC_comb_Run1.corr')
  ...
  do
   ...
   if(n.le.skip) then
    ...
    read(file_id_common3,*, iostat=ios) i, j, dble_tmp
    if(ios.ne.0) exit
    LHCrun1_correlationmatrix(i,j) = dble_tmp
   endif 
  enddo  
   
  do i=1,20
   do j=1,20
    if(LHCrun1_correlationmatrix(i,j).ne.LHCrun1_correlationmatrix(j,i)) then
     write(*,*) i, j, LHCrun1_correlationmatrix(i,j),LHCrun1_correlationmatrix(j,i)
    endif
   enddo
  enddo
    
 end subroutine setup_LHC_Run1_combination
!--------------------------------------------------------------------
