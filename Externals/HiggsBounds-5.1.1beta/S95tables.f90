...
!******************************************************************      
 ...
 !**********************************************************
  ...
  ! or taken away:  
  ntable1=193
  ntable2=35              ! table type 2 involves 2 variables
 ...
 !**********************************************************
  ...
  do j=1,npart    
   ...
   if(     (abs(mass(jj)-mass(j)).le.S95_t1(c)%deltax) &
    ...
    case(059302)
     fact(j)=t%lhc8%XS_vbf_ratio(j)*t%lhc8%XS_vbf_SM(j)*t%BR_hjZZ(j)
    case(20130132)
...
!    Data given in pb
    ...
    case(160312,2016088)
!    Data given in pb
     fact(j)= t%lhc13%XS_Hpjtb(j) * t%BR_Hpjtaunu(j)
    case(1504233)
!    Data given in fb
     fact(j)= 1000.0D0*t%lhc8%XS_vbf_Hpj(j) * t%BR_HpjWZ(j)
    case(2016089)
!    Data given in pb
       fact(j)= t%lhc13%XS_Hpjtb(j) * t%BR_Hpjtb(j)
!    Daniel's attempts
    case(2016025)
...
!      enddo
  ...
  if(fact(jj).le.vvsmall)then!A  !Higgs jj doesn't contribute - wait until another call of this subroutine before
   ...
   ! <---   
    ...
    case(04670,046701,046702)
    case(0038911,0038912,0038913,0038914)
    case(160312,2016088,2016089,14011,011811,16029,17002,14013,2016071,16002,011812,20160851,20160852)
    case(1504233,1506534)
    case(160371,160372,044781,044782,20160151,20160152,17006)
    case(20160741,20160742,2016062,20160821,20160822,20160823,20160792,20160793,2016049)
...
! (  Acceptance factor linearly increasing between 200 and 700 GeV, then constant)
  ...
  !********************************************************
   ...
   !-------------------------------------------   
   double precision, allocatable :: XS_ratio(:), ds(:)
   double precision :: s
   integer :: k
    ...
    &    20160152,16010,17002,150600424,16035)
    ...
    case(14022)
     ...
     XS_ratio=(/ div( t%lhc8%XS_hj_ratio(j),t%lhc8%XS_H_SM(j),0.0D0,1.0D9), &
               div( t%lhc8%XS_vbf_ratio(j),t%lhc8%XS_vbf_SM(j),0.0D0,1.0D9), &
               div( t%lhc8%XS_hjW_ratio(j),t%lhc8%XS_HW_SM(j),0.0D0,1.0D9), &
               div( t%lhc8%XS_hjZ_ratio(j),t%lhc8%XS_HZ_SM(j),0.0D0,1.0D9) /)
     s=sum(XS_ratio)/4.
     do k=1,4
      ds(k)= div((XS_ratio(k)-s), s, 0.0D0, 1.0D9)
     enddo
     if(S95_t2(c)%needs_M2_gt_2M1.and.(massj(j).lt.2.0D0*massi(i)))then
      test_appl=0.0D0
     elseif(massj(j).lt.massi(i))then
      test_appl=0.0D0
     elseif(maxval(ds).gt.0.1D0) then ! SM likeness test of the heavier Higgs boson
      test_appl=0.0D0
     else
      test_appl=s*x
     endif             
...
!----Check for sufficiently SM-like BRs to gaga and bb of lighter Higgs
 ...
 !******************************************************** 
  ...
  case(1812,2011138,2011151,2760,2013090,2014050,14020,8353,7712,11002,11008,160311)
  ...
  case(0038912) 
   descrip=' (p p)->VBF, h'//j//'->W W (employs NWA)' //label
  case(0038913) 
   descrip=' (p p)->h'//j//'->W W (assumes SM Higgs width, employs CPS)' //label
  case(0038914) 
   descrip=' (p p)->VBF, h'//j//'->W W (assumes SM Higgs width, employs CPS)' //label
  ...
  case(2014049,20160852,160372)
   descrip=' (p p)->bbh'//j//'->tau tau ' //label
  case(20140492,20160851,160371)
...
! Added by OS 2012-03-12
 ...
 ! -1 fails the SM-like test and Mi.lt.MSingleLim 
  use usefulbits, only : dataset,div, vsmall, iselementofarray, np, Hneut
  use theory_BRfunctions
  use theory_XS_SM_functions   
  implicit none      
  !--------------------------------------input      
  ...
  double precision :: SMrate, weight, c, dcbyc_dble
...
! This only checks the relative proportions of the four main production modes   
  ...
  case(04670) ! This is a H->h(SM)h(SM) search!
   ns = 1; nb =1; call initialise_XS_rat_BR_rat
  ...
  case(046702) ! This is a H->h(SM)h(SM) search!
   ns = 1; nb =1; call initialise_XS_rat_BR_rat
   XS_rat(1) = t%lhc8%XS_hj_ratio(j)*t%lhc8%XS_H_SM(j)
   BR_rat(1) = 0.0D0
   correct_properties=.False.
   do jj=1,np(Hneut)
    if(abs(t%particle(Hneut)%M(jj)-125.4).lt.(10.0D0+t%particle(Hneut)%dMh(jj))) then
  ...
  case(0038911) ! H-WW search with width-assumptions
   ns = 1; nb =1; call initialise_XS_rat_BR_rat
   XS_rat(1) = t%lhc8%XS_hj_ratio(j)*t%lhc8%XS_H_SM(j)
   BR_rat(1) = t%BR_hjWW(j)
   if(t%particle(Hneut)%GammaTot(j)<0.2D0*t%GammaTot_SM(j)) then ! NWA requirement
    correct_properties=.True.
   else 
    correct_properties=.False.
   endif 

  case(0038912) ! H-WW search with width-assumptions
   ns = 1; nb =1; call initialise_XS_rat_BR_rat
   XS_rat(1) = t%lhc8%XS_vbf_ratio(j)*t%lhc8%XS_vbf_SM(j)
   BR_rat(1) = t%BR_hjWW(j)
   if(t%particle(Hneut)%GammaTot(j)<0.2D0*t%GammaTot_SM(j)) then ! NWA requirement
    correct_properties=.True.
   else 
  ...
  case(0038913) ! H-WW search with width-assumptions
   ns = 1; nb =1; call initialise_XS_rat_BR_rat
   XS_rat(1) = t%lhc8%XS_hj_ratio(j)*t%lhc8%XS_H_SM(j)
   BR_rat(1) = t%BR_hjWW(j)
   if(t%particle(Hneut)%GammaTot(j)>0.8D0*t%GammaTot_SM(j)) then ! NWA requirement
    correct_properties=.True.
   else 
    correct_properties=.False.
   endif 

  case(0038914) ! H-WW search with width-assumptions
   ns = 1; nb =1; call initialise_XS_rat_BR_rat
   XS_rat(1) = t%lhc8%XS_vbf_ratio(j)*t%lhc8%XS_vbf_SM(j)
   BR_rat(1) = t%BR_hjWW(j)
   if(t%particle(Hneut)%GammaTot(j)>0.8D0*t%GammaTot_SM(j)) then ! NWA requirement
...
!      write(*,*) "Analysis application ", id, "fail the SM likeness test!"
     endif
    enddo 
