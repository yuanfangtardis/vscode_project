...
!********************************************************
 ...
 !********************************************************
  ...
  !-----------------------------------internal
  integer :: i                    
  character(len=300) :: start_of_filename !must be same size as infile1 or bigger
  !-------------------------------------------
  ...
  case('effC','part','hadr')
   ...
   do x=1,ubound(pr,dim=1)
    ...
    if(n.ge.1)then
      proc=pr(x)   
      write(tempchar,'(I3)')x
      if(proc%ttype.eq.1.or.proc%ttype.eq.2) then                
      call outputproc(proc,file_id_common,descrip,1)
      elseif(proc%ttype.eq.9) then
       call outputproc_llh(proc,file_id_common,descrip)
      endif      
...
!      call weboutput(res(jj))
 ...
 !********************************************************
  ...
  135   format('process ',1I4,' (DEACTIVATED BY THE USER!)')
  ...
  do n=1,ntot    
   ...
   if(is_analysis_deactivated(tableid)) then
    write(file_id_common,135)n
   else 
   ...
   elseif(pr(n)%ttype.eq.9) then
    call outputproc_llh(pr(n),file_id_common,descrip)
    write(file_id_common,*)trim(descrip)   
   else 
